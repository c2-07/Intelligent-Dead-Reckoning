...
from train_sota_2025 import SOTA_2025_Net

print("Loading data for the SOTA 2025 Benchmark...")
...
print("Loading SOTA 2025 AI Model (Ackermann + DUET)...")
model = SOTA_2025_Net(in_channels=6)
try:
    model.load_state_dict(torch.load("sota_2025_model.pth", map_location='cpu', weights_only=True))
except:
    model.load_state_dict(torch.load("sota_2025_model.pth", map_location='cpu'))
model.eval()
...
with torch.no_grad():
    for i in range(window_size - 1, len(s_val)):
        ...
        # SOTA_2025_Net returns v_pred, bias_pred. 
        # v_pred is shape (Batch, Seq). We take the last frame.
        v_pred, bias_pred = model(x_tensor)
        pred = v_pred[0, -1].item()
...
# We fetch it safely using iloc column 21 as found in previous experiments
absolute_heading = np.radians(s_val.iloc[:, 21].values)

print("\nRunning Sensor-Fused Kinematic Benchmark (< 10% Target)...")
