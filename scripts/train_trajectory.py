...
class SequenceIDRDataset(Dataset):
    def __init__(self, feature_list, label_list, window_size=50):
        ...
        for feats, lbls in zip(feature_list, label_list):
            ...
            for i in range(0, len(feats) - window_size, step):
                ...
                # WE NOW STORE THE ENTIRE SEQUENCE OF VELOCITIES, NOT JUST THE LAST FRAME
...
class Trajectory_ResNet_LSTM(nn.Module):
    def __init__(self, in_channels=6):
        super().__init__()
        self.in_channels = 32
        self.conv1 = nn.Conv1d(in_channels, 32, kernel_size=7, stride=1, padding=3, bias=False) # Changed stride to 1 to preserve seq len
    ...
    def forward(self, x):
        ...
        # PREDICT VELOCITY FOR EVERY TIME STEP IN THE SEQUENCE
        # out is (Batch, Seq, 128)
        return self.fc(out).squeeze(-1) # (Batch, Seq)

def trajectory_loss(v_pred, v_true):
    criterion = nn.HuberLoss(delta=1.0)
    
    # 1. Instantaneous Velocity Loss (Are we guessing the right speed right now?)
    loss_vel = criterion(v_pred, v_true)
    
    # 2. Distance Trajectory Loss (Are our errors canceling each other out?)
    # dt = 0.1 seconds per frame. Total distance = sum(v * dt)
    dist_pred = torch.sum(v_pred * 0.1, dim=1)
    dist_true = torch.sum(v_true * 0.1, dim=1)
    loss_dist = criterion(dist_pred, dist_true)
    
    # Combine them. Heavily penalize distance drift.
    return loss_vel + (2.0 * loss_dist), loss_vel, loss_dist

def main():
    print("Starting Trajectory Loss Training Pipeline")
    pull_all_data()
    ...
    train_dataset = SequenceIDRDataset(train_feats, train_lbls)
    val_dataset = SequenceIDRDataset(val_feats, val_lbls)
    train_loader = DataLoader(train_dataset, batch_size=256, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=256, shuffle=False)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = Trajectory_ResNet_LSTM().to(device)
    ...
    for epoch in range(epochs):
        ...
        for x, y in train_loader:
            ...
            v_pred = model(x)
            loss, l_vel, l_dist = trajectory_loss(v_pred, y)
        ...
        # Since v_pred and y are sequences, we average over both batch and seq for RMSE calculation
        train_rmse = np.sqrt(train_v_loss / len(train_loader) * 2) * 3.6
        val_rmse = np.sqrt(val_v_loss / len(val_loader) * 2) * 3.6
        print(f"Epoch {epoch+1:02d} | Train RMSE: {train_rmse:.2f} km/h | Val RMSE: {val_rmse:.2f} km/h | Val Total Loss: {val_loss:.4f}")
        
        if val_loss < best_loss:
            best_loss = val_loss
            torch.save(model.state_dict(), 'trajectory_model.pth')
            
    print(f"Done in {(time.time()-start_time)/60:.1f} mins. Saved trajectory_model.pth")
