...
def pull_all_data():
    ...
    if not os.path.exists("IO-VNBD"):
        ...
        subprocess.run(
            "cd IO-VNBD && git lfs install && git lfs pull --include='**/*.csv'",
            shell=True,
        )
...
def process_single_pair(v_path, s_path, lag_samples=9):
    ...
    try:
        ...
        gx, gy, gz = (
            s_aligned["GRAVITY X (m/s²)"].values,
            s_aligned["GRAVITY Y (m/s²)"].values,
            s_aligned["GRAVITY Z (m/s²)"].values,
        )
        ax, ay, az = (
            s_aligned["ACCELEROMETER X (m/s²)"].values,
            s_aligned["ACCELEROMETER Y (m/s²)"].values,
            s_aligned["ACCELEROMETER Z (m/s²)"].values,
        )
        groll, gpitch, gyaw = (
            s_aligned["GYROSCOPE Roll (rad/s)"].values,
            s_aligned["GYROSCOPE Pitch (rad/s)"].values,
            s_aligned["GYROSCOPE Yaw (rad/s)"].values,
        )

        rot_ax, rot_ay, rot_az = np.zeros_like(ax), np.zeros_like(ay), np.zeros_like(az)
        rot_groll, rot_gpitch, rot_gyaw = (
            np.zeros_like(groll),
            np.zeros_like(gpitch),
            np.zeros_like(gyaw),
        )
        ...
        for i in range(len(s_aligned)):
            ...
            rot_ax[i], rot_ay[i], rot_az[i] = rot_matrix.dot(
                np.array([ax[i], ay[i], az[i]])
            )
            rot_groll[i], rot_gpitch[i], rot_gyaw[i] = rot_matrix.dot(
                np.array([groll[i], gpitch[i], gyaw[i]])
            )

        features = np.column_stack(
            (rot_ax, rot_ay, rot_az, rot_groll, rot_gpitch, rot_gyaw)
        )
...
def load_all_data(
    ...
    for folder in folders:
        ...
        if exclude_trip and exclude_trip in folder_name:
            continue
        if include_trip and include_trip not in folder_name:
            continue
            
        v_files = sorted(
            glob.glob(os.path.join(folder, "**", "V-*.csv"), recursive=True)
        )
        s_files = sorted(
            glob.glob(os.path.join(folder, "**", "S-*.csv"), recursive=True)
        )
